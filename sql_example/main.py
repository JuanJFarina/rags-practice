...
file_url = "employees.csv"
data = pd.read_csv(file_url).fillna(value = 0)  # type: ignore
data.to_sql(
    'employees',
    con=engine,
    if_exists='replace',
...
db = SQLDatabase.from_uri(f'sqlite:///{database_file_path}')  # type: ignore
toolkit = SQLDatabaseToolkit(db=db, llm=model)

MESSAGE = "Who is the employee with id 4 ?"
...
result = agent_executor_SQL.invoke(MESSAGE)  # type: ignore
